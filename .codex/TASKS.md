...
| T-014 | Make local VAD explicit and default-off to preserve subtitle timing gaps | done | agent | - | `transcribe_only` no longer fills long silence gaps by default; local CLI exposes `--vad/--no-vad`; verification fixture reproduces improvement on the known `非常欢迎大家` segment |
